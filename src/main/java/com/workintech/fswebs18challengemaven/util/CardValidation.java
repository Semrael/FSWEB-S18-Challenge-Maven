...
public class CardValidation {
    ...
    public static void validateCard(Card card){
        ...
        if(card.getType()!=null&&card.getType().equals(Type.JOKER)&&((card.getValue()!=null)
                || (card.getColor()!=null&& card.getValue()>0))){
            throw new CardException("Eğer kart tipi ```JOKER``` iser hem  ```value``` hem de ```color``` değerleri null olmalıdır.",HttpStatus.BAD_REQUEST);
        }
    }
}
